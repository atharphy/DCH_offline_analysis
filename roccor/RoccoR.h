...
// boost/math/special_functions/erf.hpp intentionally removed: it fails to
// parse under ROOT's ACLiC/rootcling (boost's C++11 feature-detection
// misfires inside cling, hard-erroring via cxx03_warn.hpp) and the only
// caller, CrystalBall::invcdf(), is exclusively used by RocRes::kSmear /
// kExtra (i.e. RoccoR::kSmearMC / kGenSmear), which this framework does not
// use (we use kScaleDT / kScaleMC / kSpreadMC's 3-arg kSpread, none of which
// touch CrystalBall at all). invcdf() now throws instead of silently
// returning a wrong value if ever reached.
#include <stdexcept>
...
struct CrystalBall{
	...
	throw std::runtime_error("CrystalBall::invcdf: boost::math::erf_inv was intentionally removed (see RoccoR.h); this path is only reachable via kSmearMC/kGenSmear, which this framework does not use.");
    }
};
...
class RoccoR{
	...
	double kScaleMCerror(int Q, double pt, double eta, double phi) const;
