...
const std::string kOutputBase = "../../Dependencies/systematics/results/dy_mc";

bool processOne(const std::string& period, const std::string& region, TFile& out) {
    auto data = loadDataRate(period, region);
    auto mc = loadDYMCRate(period, region);
    if (!data || !mc) { std::cerr << "Skipping " << period << " " << region << " (missing DY data or DY MC input)" << std::endl; return false; }
    const std::string base = "FR_DY_" + period + "_" + region;
    std::unique_ptr<TH1D> up, down;
    makeSymmetricVariation(*data, *mc, base + "_up", base + "_down", up, down);
    out.cd();
...
void ComputeFRSystematic_DYMC() {
    ...
    for (const std::string& period : periods) {
        ...
        const std::string outPath = outDir + "/tau_fake_rate_syst_dy_mc_" + period + ".root";
    ...
    std::cout << "Wrote " << written << " of " << (periods.size() * 2) << " (period, region) DY-MC systematic variations under " << kOutputBase << std::endl;
}
