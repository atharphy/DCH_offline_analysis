...
const std::string kOutputBase = "../../Dependencies/systematics/results/wjets_data";

bool processOne(const std::string& period, const std::string& region, TFile& out) {
    auto data = loadDataRate(period, region);
    auto wjData = loadWJetsDataRate(period, region);
    if (!data || !wjData) { std::cerr << "Skipping " << period << " " << region << " (missing DY data or W+jets data input)" << std::endl; return false; }
    const std::string base = "FR_DY_" + period + "_" + region;
    std::unique_ptr<TH1D> up, down;
    makeSymmetricVariation(*data, *wjData, base + "_up", base + "_down", up, down);
    out.cd();
...
void ComputeFRSystematic_WJetsData() {
    ...
    for (const std::string& period : periods) {
        ...
        const std::string outPath = outDir + "/tau_fake_rate_syst_wjets_data_" + period + ".root";
    ...
    std::cout << "Wrote " << written << " of " << (periods.size() * 2) << " (period, region) W+jets-data systematic variations under " << kOutputBase << std::endl;
}
