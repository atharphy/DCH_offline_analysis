...
const std::string kOutputBase = "../../Dependencies/systematics/results/tt_mc";

bool processOne(const std::string& period, const std::string& region, TFile& out) {
    auto data = loadDataRate(period, region);
    auto ttMc = loadTTMCRate(period, region);
    if (!data || !ttMc) { std::cerr << "Skipping " << period << " " << region << " (missing DY data or TT MC input)" << std::endl; return false; }
    const std::string base = "FR_DY_" + period + "_" + region;
    std::unique_ptr<TH1D> up, down;
    makeSymmetricVariation(*data, *ttMc, base + "_up", base + "_down", up, down);
    out.cd();
...
void ComputeFRSystematic_TtMc() {
    ...
    for (const std::string& period : periods) {
        ...
        const std::string outPath = outDir + "/tau_fake_rate_syst_tt_mc_" + period + ".root";
    ...
    std::cout << "Wrote " << written << " of " << (periods.size() * 2) << " (period, region) TT-MC systematic variations under " << kOutputBase << std::endl;
}
