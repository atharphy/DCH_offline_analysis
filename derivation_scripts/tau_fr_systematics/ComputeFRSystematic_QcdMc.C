...
const std::string kOutputBase = "../../Dependencies/systematics/results/qcd_mc";

bool processOne(const std::string& period, const std::string& region, TFile& out) {
    auto data = loadDataRate(period, region);
    auto qcdMc = loadQCDMCRate(period, region);
    if (!data || !qcdMc) { std::cerr << "Skipping " << period << " " << region << " (missing DY data or QCD MC input)" << std::endl; return false; }
    const std::string base = "FR_DY_" + period + "_" + region;
    std::unique_ptr<TH1D> up, down;
    makeSymmetricVariation(*data, *qcdMc, base + "_up", base + "_down", up, down);
    out.cd();
...
void ComputeFRSystematic_QcdMc() {
    ...
    for (const std::string& period : periods) {
        ...
        const std::string outPath = outDir + "/tau_fake_rate_syst_qcd_mc_" + period + ".root";
    ...
    std::cout << "Wrote " << written << " of " << (periods.size() * 2) << " (period, region) QCD-MC systematic variations under " << kOutputBase << std::endl;
}
